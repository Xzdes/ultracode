...
#[inline] fn ec_to_bits(ec: EcLevel) -> u16 {
    match ec { EcLevel::M => 0b00, EcLevel::L => 0b01, EcLevel::H => 0b10, EcLevel::Q => 0b11 }
}
fn bch15_5_encode(info5: u16) -> u16 {
    let mut v = info5 << 10;
    let mut msb = 14;
    while msb >= 10 {
        if (v & (1<<msb)) != 0 { v ^= BCH_FORMAT_GEN << (msb-10); }
        if msb == 0 { break; }
        msb -= 1;
    }
    (info5 << 10) | (v & 0x03FF)
}
#[inline] fn encode_format_bits(ec: EcLevel, mask_id: u8) -> u16 {
...
pub fn synthesize_qr_v1_from_text(text: &str, mask_id: u8, unit: usize) -> GrayImage<'static> {
    ...
    assert!(bytes.len() <= 17, "v1-L Byte mode влезает до 17 байт данных");
    let mut bits: Vec<bool> = Vec::new();
    // mode 0100
    for i in (0..4).rev() { bits.push(((0b0100 >> i) & 1) != 0); }
    // length 8
    for i in (0..8).rev() { bits.push((((bytes.len() as u32) >> i) & 1) != 0); }
    // payload
    for &b in bytes {
        for i in (0..8).rev() { bits.push(((b as u32 >> i) & 1) != 0); }
    }
    ...
    for _ in 0..term { bits.push(false); }
    // до байтовой границы
    while bits.len() % 8 != 0 { bits.push(false); }
    // Пад-кодворды 0xEC, 0x11
    let mut data_cw: Vec<u8> = {
        let mut out = Vec::new();
        for chunk in bits.chunks(8) {
            let mut b = 0u8;
            for &bit in chunk { b = (b << 1) | if bit {1} else {0}; }
            out.push(b);
        }
        while out.len() < 19 {
    ...
    // Copy A: y=8, x=0..=8 (кроме 6); x=8, y=8..=0 (кроме 8 и 6)
    {
        ...
        for x in 0..=8 { if x != 6 { put(x, 8); } }
        for y in (0..=8).rev() { if y != 8 && y != 6 { put(8, y); } }
    }
    // Copy B: y=8, x=20..=13; x=8, y=20..=14
    {
        ...
        for x in (13..=20).rev() { put(x, 8); }
        for y in (14..=20).rev() { put(8, y); }
    }

    // 4) Размещение данных по «змейке» с применением маски только для data-модулей.
    let mut bit_iter = all_cw.iter().flat_map(|&cw| (0..8).rev().map(move |i| ((cw >> i) & 1) != 0));
        ...
        for _sy in 0..unit {
            for mx in 0..total {
                let val = if (qz..qz+21).contains(&mx) && (qz..qz+21).contains(&my) {
                    ...
                    let gy = my - qz;
                    grid[gy*21 + gx]
                } else { false }; // quiet = белый
                let px = if val { 0u8 } else { 255u8 };
                for _sx in 0..unit { data.push(px); }
            }
        }
    }
